...
class UserDeleteMessageTest(unittest.TestCase):
    ...
    def testDeleteLoginMessage(self):
        """
        Test if user can delete last login message.
        """
        with app.app_context():
            user = self.amc.decrypt_by_username(username="test")
            self.assertEqual(self.mmc.del_messasge(self.mmc.send_last_login(user)), None)

    def testDeleteWelcomeMessage(self):
        """
        Test if user can delete welcome message.
        """
        with app.app_context():
            user = self.amc.decrypt_by_username(username="test")
            self.assertEqual(self.mmc.del_messasge(self.mmc.send_welcome_msg(100, user)), None)
